...
// cache版本的矩阵乘向量，按行顺序访问，提高cache命中率
void cache(const vector<int>& A, const vector<int>& x, vector<int>& y, int n) {
...
Row run_one(int n, const LARGE_INTEGER& freq) {
    ...
    init_data(A, x, n);
    cache(A, x, y, n); // warm-up，预热后再正式计时
...
// g++ -O2 matrix_cache.cpp -o matrix_cache
