...
// 超标量优化版本：两路链式累加
__declspec(noinline) int64_t super(const vector<int>& a, int n) {
    int64_t s1 = 0, s2 = 0;
    int i = 0;
...
Row run_one(int n, const LARGE_INTEGER& freq) {
    ...
    init_data(a, n);
    int64_t warm = super(a, n); // warm-up，预热后再正式计时
...
// g++ -O2 sum_super.cpp -o sum_super
